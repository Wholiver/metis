...
import type { CollaborationMode, MemoryState, ModelOption, ProjectItem } from '../../types';
...
export function SettingsDialog(props: SettingsDialogProps) {
  ...
  const [providerForm, setProviderForm] = useState<ProviderConfig>({ reasoning: true });
  ...
  const model = <><SectionHeading title="Models" description="Select the current model and reasoning level, then set defaults for new sessions." /><Card>
    ...
    <Row label="Reasoning effort" description="Availability depends on the current model."><select className={selectClass} value={props.thinkingLevel} onChange={(e) => void props.onSelectThinkingLevel(e.target.value)} disabled={disabled || !props.supportsThinking}>{props.thinkingLevels.map((level) => <option key={level} value={level}>{level}</option>)}</select></Row>
