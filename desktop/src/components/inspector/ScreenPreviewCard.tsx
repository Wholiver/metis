...
export const ScreenPreviewCard: React.FC<ScreenPreviewCardProps> = ({ agent }) => {
  return (
    <div className="flex flex-col gap-2.5">
      <div className="w-full bg-[#f1f3f6] dark:bg-[#1a1d24] rounded-[16px] border border-slate-200/90 dark:border-[#272b36] overflow-hidden shadow-[0_2px_8px_rgba(0,0,0,0.04)] p-2">
        {/* Chrome Browser Window */}
        <div className="w-full rounded-[12px] overflow-hidden bg-white dark:bg-[#16171a] border border-slate-300/60 dark:border-[#272b36] shadow-sm">
          {/* Chrome Header */}
          <div className="h-6 bg-[#f8fafc] dark:bg-[#1f232b] border-b border-slate-200 dark:border-[#272b36] flex items-center px-2.5 gap-1.5">
            <span className="w-2 h-2 rounded-full bg-[#ff5f56]" />
            <span className="w-2 h-2 rounded-full bg-[#ffbd2e]" />
            <span className="w-2 h-2 rounded-full bg-[#27c93f]" />

            <div className="flex-1 text-center pr-3">
              <span className="text-[9.5px] text-[#94a3b8] dark:text-slate-400 font-sans">
                calendar.google.com
              </span>
            </div>
          </div>
          ...
          {/* Calendar Canvas */}
          <div className="h-[145px] bg-white dark:bg-[#16171a] relative p-2 flex gap-2 overflow-hidden select-none">
            {/* Left mini sidebar calendar */}
            <div className="w-8 h-full bg-[#f8fafc] dark:bg-[#1f232b] rounded p-1 flex flex-col gap-1 border border-slate-100 dark:border-[#272b36]">
              <div className="w-full h-1.5 bg-slate-300 dark:bg-slate-600 rounded-[1px]" />
              <div className="grid grid-cols-3 gap-0.5 mt-0.5">
                {[...Array(9)].map((_, i) => (
                  <div key={i} className="w-1.5 h-1.5 bg-slate-200 dark:bg-slate-700 rounded-[1px]" />
                ))}
              </div>
            </div>

            {/* Main Calendar Grid */}
            <div className="flex-1 h-full grid grid-cols-4 gap-1 relative border-l border-slate-100 dark:border-[#272b36] pl-1.5">
              {/* Col 1 */}
