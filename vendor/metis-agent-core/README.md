...
Stateful agent with tool execution and event streaming. Built on `@earendil-works/metis-ai`.

## Installation

```bash
npm install @earendil-works/metis-agent-core
```
...
```typescript
declare module "@earendil-works/metis-agent-core" {
...
```typescript
import { Agent, streamProxy } from "@earendil-works/metis-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@earendil-works/metis-agent-core";
