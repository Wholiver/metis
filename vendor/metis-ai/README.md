...
```bash
npm install @earendil-works/metis-ai
```

TypeBox exports are re-exported from `@earendil-works/metis-ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@earendil-works/metis-ai';
import { builtinModels } from '@earendil-works/metis-ai/providers/all';
...
```typescript
import { anthropicProvider } from '@earendil-works/metis-ai/providers/anthropic';
import { openaiProvider } from '@earendil-works/metis-ai/providers/openai';
import { openrouterProvider } from '@earendil-works/metis-ai/providers/openrouter';
import { amazonBedrockProvider } from '@earendil-works/metis-ai/providers/amazon-bedrock';
...
```typescript
import { hasApi } from '@earendil-works/metis-ai';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@earendil-works/metis-ai/providers/all';
...
Stored credentials (API keys entered interactively, OAuth tokens) live in a `CredentialStore` — one type-tagged credential per provider. metis-ai ships an in-memory default; apps inject persistent storage:

```typescript
import { createModels, type CredentialStore } from '@earendil-works/metis-ai';
...
API-key credentials use the same discriminator as metis's `auth.json` and can carry provider-scoped env/config values:
...
```typescript
import { Type, type Tool, StringEnum } from '@earendil-works/metis-ai';
...
```typescript
import { validateToolCall, type Tool } from '@earendil-works/metis-ai';
...
```typescript
import { builtinImagesModels } from '@earendil-works/metis-ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@earendil-works/metis-ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@earendil-works/metis-ai';
import { openAICompletionsApi } from '@earendil-works/metis-ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from '@earendil-works/metis-ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from '@earendil-works/metis-ai/api/openai-responses.lazy';
...
Use model-level `thinkingLevelMap` to describe model-specific thinking controls. Keys are metis thinking levels (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`). Missing keys use provider defaults, string values are sent to the provider, and `null` marks a level unsupported.
...
```typescript
import { stream } from '@earendil-works/metis-ai/api/anthropic-messages';
...
interface OpenAICompletionsCompat {
  ...
  chatTemplateKwargs?: Record<string, string | number | boolean | null | { '$var': 'thinking.enabled' | 'thinking.effort'; omitWhenOff?: boolean }>; // chat_template_kwargs values; use $var for metis-controlled thinking values
...
```typescript
import { createModels, type Context } from '@earendil-works/metis-ai';
import { anthropicProvider } from '@earendil-works/metis-ai/providers/anthropic';
import { openaiProvider } from '@earendil-works/metis-ai/providers/openai';
import { googleProvider } from '@earendil-works/metis-ai/providers/google';
...
```typescript
import { createModels } from '@earendil-works/metis-ai';
import { anthropicProvider } from '@earendil-works/metis-ai/providers/anthropic';
...
- `@earendil-works/metis-ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@earendil-works/metis-ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@earendil-works/metis-ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@earendil-works/metis-ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@earendil-works/metis-ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@earendil-works/metis-ai/api/bedrock-converse-stream.lazy';
import { bedrockProviderModule } from '@earendil-works/metis-ai/bedrock-provider';
...
Pass `env` in stream options to scope provider configuration to a request. Values in `env` are used before process environment variables for provider auth and configuration such as Cloudflare account IDs, Azure OpenAI settings, Vertex project/location, Bedrock settings, `METIS_CACHE_RETENTION`, and `HTTP_PROXY`/`HTTPS_PROXY`.
...
```bash
npx @earendil-works/metis-ai login              # interactive provider selection
npx @earendil-works/metis-ai login anthropic    # login to specific provider
npx @earendil-works/metis-ai list               # list available providers
```
...
The legacy flow functions remain available via the `@earendil-works/metis-ai/oauth` entry point (`loginAnthropic`, `loginOpenAICodex`, `loginGitHubCopilot`, `refreshOAuthToken`, `getOAuthApiKey`); credential storage is the caller's responsibility there. New code should prefer the provider-owned `OAuthAuth` shown above — it composes with the credential store and gets locked auto-refresh for free.
...
```typescript
// Before
import { getModel, complete } from '@earendil-works/metis-ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@earendil-works/metis-ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@earendil-works/metis-ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@earendil-works/metis-ai`.
