...
export default function (metis: ExtensionAPI) {
	...
	metis.on("session_start", async (event, ctx) => {
		ctx.ui.setTitle(event.reason === "new" ? "Metis RPC Demo (new session)" : "Metis RPC Demo");
	...
	metis.on("turn_start", async (_event, ctx) => {
		turnCount++;
		ctx.ui.setStatus("rpc-demo", `Turn ${turnCount} running...`);
	});

	metis.on("turn_end", async (_event, ctx) => {
	...
	metis.registerCommand("rpc-input", {
	...
	metis.registerCommand("rpc-editor", {
	...
	metis.registerCommand("rpc-prefill", {
