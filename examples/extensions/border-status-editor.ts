...
import type { Component, EditorTheme, TUI } from "@earendil-works/metis-tui";
import { truncateToWidth, visibleWidth } from "@earendil-works/metis-tui";
...
export default function (metis: ExtensionAPI) {
	...
	metis.on("session_start", (_event, ctx) => {
		...
		const refreshBranch = async () => {
			const result = await metis.exec("git", ["branch", "--show-current"], { cwd: ctx.cwd }).catch(() => undefined);
		...
		class BorderStatusEditor extends CustomEditor {
			...
			render(width: number): string[] {
				...
				const thinking = metis.getThinkingLevel();
