 ...
 *
 * Runs Metis built-in tools inside a local Gondolin micro-VM. The host working
 ...
 * Usage:
 *   cd /path/to/project
 *   metis -e /path/to/metis/packages/coding-agent/examples/extensions/gondolin
 *
 * Requirements:
...
export default function (metis: ExtensionAPI) {
	...
	async function startVm(ctx?: ExtensionContext): Promise<VM> {
		...
		const created = await VM.create({
			sessionLabel: `metis ${path.basename(localCwd)}`,
			vfs: {
				mounts: {
	...
	metis.registerCommand("gondolin", {
	...
	metis.on("user_bash", async (_event, ctx) => {
		const activeVm = await ensureVm(ctx);
