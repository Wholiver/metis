...
import type { ExtensionAPI, SessionBeforeSwitchEvent, SessionMessageEntry } from "metis";

export default function (metis: ExtensionAPI) {
	metis.on("session_before_switch", async (event: SessionBeforeSwitchEvent, ctx) => {
		if (!ctx.hasUI) return;
