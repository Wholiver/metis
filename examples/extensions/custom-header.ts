 ...
 * (logo + keybinding hints) with a custom component showing the Metis mascot.
 */

import type { ExtensionAPI, Theme } from "metis";
import { VERSION } from "metis";

// --- Metis MASCOT ---
// Based on the Metis agent character
function getMetisMascot(theme: Theme): string[] {
	// --- COLORS ---
	// 3b1b Blue: R=80, G=180, B=230
	const metisBlue = (text: string) => theme.fg("accent", text);
	...
	const lineBar = `  ${metisBlue(BLOCK.repeat(14))}`;

	// 4. Lines 3-6: The Legs
	// Indented 5 spaces relative to the very left edge
	// Leg width: 2 blocks | Gap: 4 blocks
	const lineLeg = `     ${metisBlue(BLOCK.repeat(2))}    ${metisBlue(BLOCK.repeat(2))}`;
...
export default function (metis: ExtensionAPI) {
			...
			ctx.ui.setHeader((_tui, theme) => {
				return {
					render(_width: number): string[] {
						const mascotLines = getMetisMascot(theme);
	...
	metis.registerCommand("builtin-header", {
