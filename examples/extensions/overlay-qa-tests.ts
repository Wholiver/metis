 ...
 *
 * Usage: metis --extension ./examples/extensions/overlay-qa-tests.ts
 *
 * Commands:
...
import type { ExtensionAPI, ExtensionCommandContext, Theme } from "metis";
import type { Component, OverlayAnchor, OverlayHandle, OverlayOptions, TUI } from "@earendil-works/metis-tui";
import { Input, matchesKey, truncateToWidth, visibleWidth } from "@earendil-works/metis-tui";
...
export default function (metis: ExtensionAPI) {
	// Animation demo - proves overlays can handle real-time updates (like Metis-doom would need)
	metis.registerCommand("overlay-animation", {
	...
	metis.registerCommand("overlay-anchors", {
	...
	metis.registerCommand("overlay-margins", {
	...
	metis.registerCommand("overlay-stack", {
	...
	metis.registerCommand("overlay-overflow", {
	...
	metis.registerCommand("overlay-edge", {
	...
	metis.registerCommand("overlay-percent", {
	...
	// Test maxHeight
	metis.registerCommand("overlay-maxheight", {
	...
	metis.registerCommand("overlay-sidepanel", {
	...
	metis.registerCommand("overlay-toggle", {
	...
	metis.registerCommand("overlay-passive", {
	...
	metis.registerCommand("overlay-focus", {
	...
	metis.registerCommand("overlay-streaming", {
	...
	private startProcess(): void {
		...
		this.proc = spawn("bash", [
			...
			for i in $(seq 1 100); do
				...
				DIR="/Users/nicobailon/Documents/development/Metis-mono/packages/coding-agent/src/modes/interactive"
				...
				if [ $((i % 5)) -eq 0 ]; then
					echo -e "  \\033[32m✓ Successfully processed \${i} files in /Users/nicobailon/Documents/development/Metis-mono\\033[0m"
				fi
				if [ $((i % 7)) -eq 0 ]; then
...
// Animation demo - proves overlays can handle real-time updates like Metis-doom
class AnimationDemoComponent extends BaseOverlay {
	...
	render(width: number): string[] {
		...
		lines.push(border("│") + padLine(th.fg("dim", " (Metis-doom uses same approach)")) + border("│"));
		lines.push(border("│") + padLine(``) + border("│"));
