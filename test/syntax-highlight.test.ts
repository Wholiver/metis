import { resetCapabilitiesCache, setCapabilities } from "@earendil-works/metis-tui";
...
describe("theme syntax highlighting", () => {
	...
	it("colors diff additions and deletions in fenced diff blocks", () => {
		...
		expect(lines[0]).toBe("\x1b[38;2;201;109;120m-old\x1b[39m");
		expect(lines[1]).toBe("\x1b[38;2;143;191;136m+new\x1b[39m");
	});
	...
	it("keeps cli-highlight default styled scopes mapped to theme styles", () => {
		expect(highlightCode("const re = /foo+/gi;", "javascript")[0]).toContain(
			"\x1b[38;2;185;141;123m/foo+/gi\x1b[39m",
		);
		expect(highlightCode("@decorator", "python")[0]).toBe("\x1b[38;2;123;132;156m@decorator\x1b[39m");
