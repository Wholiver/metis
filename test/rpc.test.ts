...
import type { AgentEvent } from "@earendil-works/metis-agent-core";
...
describe.skipIf(!process.env.ANTHROPIC_API_KEY && !process.env.ANTHROPIC_OAUTH_TOKEN)("RPC mode", () => {
	...
	beforeEach(() => {
		sessionDir = join(tmpdir(), `metis-rpc-test-${Date.now()}`);
		client = new RpcClient({
			cliPath: join(__dirname, "..", "dist", "cli.js"),
			cwd: join(__dirname, ".."),
			env: { METIS_CODING_AGENT_DIR: sessionDir },
			provider: "anthropic",
	...
	test("should set and get session name", async () => {
		...
		// Send a prompt first so the first user message creates the session file.
		await client.promptAndWait("Reply with just 'ok'");
