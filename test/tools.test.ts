...
describe("Coding Agent Tools", () => {
	...
	describe("log tool", () => {
		function createLogTestContext(sessionId: string) {
			return {
				cwd: testDir,
				mode: "tui" as const,
				hasUI: true,
		...
		it("should append to current session temp log", async () => {
			...
			const logTool = (await import("../src/core/tools/log.ts")).createLogToolDefinition(testDir);
		...
		it("should insert separator when log already exists", async () => {
			...
			writeFileSync(tempLogPath, "first line\n");
			const logTool = (await import("../src/core/tools/log.ts")).createLogToolDefinition(testDir);
		...
		describe("user_intent tool", () => {
			function createUserIntentTestContext(sessionId: string) {
				return {
					cwd: testDir,
					mode: "tui" as const,
					hasUI: true,
