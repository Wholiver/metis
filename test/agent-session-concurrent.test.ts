...
describe("AgentSession concurrent prompt guard", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `metis-concurrent-test-${Date.now()}`);
	...
	it("should queue extension-origin steering messages while streaming", async () => {
		...
		const extensionsResult = await createTestExtensionsResult([
			(metis) => {
				(globalThis as typeof globalThis & { testExtensionApi?: unknown }).testExtensionApi = metis;
			},
			(metis) => {
		...
		metis!.sendUserMessage("Steer from extension", { deliverAs: "steer" });
