...
describe('desktop React settings', () => {
  ...
  it('wires migrated Provider, OAuth, Memory, workspace, and transfer controls end to end', () => {
    ...
    expect(modal).toContain("type WizardStep = 'pick' | 'connect' | 'custom-basics' | 'custom-models' | 'builtin-models'");
    expect(modal).toContain('continueBuiltinConnect');
    expect(modal).toContain('handleOAuthConnect');
