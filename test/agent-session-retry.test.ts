...
import { Agent, type AgentEvent, type AgentTool } from "@earendil-works/metis-agent-core";
import { type AssistantMessage, type AssistantMessageEvent, EventStream, getModel } from "@earendil-works/metis-ai/compat";
...
describe("AgentSession retry", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `metis-retry-test-${Date.now()}`);
