...
describe("regression #3592: no-builtin-tools keeps extension tools enabled", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `metis-no-builtin-tools-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("still disables all tools when noTools is all", async () => {
		...
		expect(session.systemPrompt).toContain("Available tools for this step:\n(none)");
		session.dispose();
	});
