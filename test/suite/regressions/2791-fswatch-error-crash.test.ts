...
/**
 * Regression test for https://github.com/earendil-works/metis/issues/2791
 *
...
describe("issue #2791 fs.watch error event crashes process", () => {
	...
	beforeEach(() => {
		tempRoot = mkdtempSync(join(tmpdir(), "metis-2791-"));
...
`,
		...
		let exitCode: number;
		try {
			_stdout = execFileSync(process.execPath, ["--experimental-strip-types", scriptPath], {
				timeout: 10000,
				encoding: "utf-8",
				env: { ...process.env, METIS_CODING_AGENT_DIR: agentDir },
