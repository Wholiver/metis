> metis can create TUI components. Ask it to build one for your use case.

# TUI Components

Extensions and custom tools can render custom TUI components for interactive user interfaces. This page covers the component system and available building blocks.

**Source:** [`@earendil-works/metis-tui`](https://github.com/earendil-works/metis/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@earendil-works/metis-tui";
...
```typescript
import { Container, type Focusable, Input } from "@earendil-works/metis-tui";
...
**In custom tools** via `metis.ui.custom()`:

```typescript
async execute(toolCallId, params, onUpdate, ctx, signal) {
  const handle = metis.ui.custom(myComponent);
  // ...
  handle.close();
}
```
...
Import from `@earendil-works/metis-tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@earendil-works/metis-tui";
```
...
```typescript
import { matchesKey, Key } from "@earendil-works/metis-tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@earendil-works/metis-tui";
...
```typescript
import { getMarkdownTheme } from "metis";
import { Markdown } from "@earendil-works/metis-tui";
...
Set `METIS_TUI_WRITE_LOG` to capture the raw ANSI stream written to stdout.

```bash
METIS_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx packages/tui/test/chat-simple.ts
```
...
For letting users pick from a list of options. Use `SelectList` from `@earendil-works/metis-tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "metis";
import { DynamicBorder } from "metis";
import { Container, type SelectItem, SelectList, Text } from "@earendil-works/metis-tui";
...
```typescript
import { BorderedLoader } from "metis";

metis.registerCommand("fetch", {
  handler: async (_args, ctx) => {
    const result = await ctx.ui.custom<string | null>((tui, theme, _kb, done) => {
...
For toggling multiple settings. Use `SettingsList` from `@earendil-works/metis-tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "metis";
import { Container, type SettingItem, SettingsList, Text } from "@earendil-works/metis-tui";

metis.registerCommand("settings", {
  handler: async (_args, ctx) => {
...
Customize the inline working indicator shown while metis is streaming a response.
...
// Restore metis's default spinner
ctx.ui.setWorkingIndicator();
```
...
import { matchesKey, truncateToWidth } from "@earendil-works/metis-tui";
