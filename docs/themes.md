> metis can create themes. Ask it to build one for your setup.
...
Metis loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.metis/agent/themes/*.json`
- Project: `.metis/themes/*.json` (only after the project is trusted)
- Packages: `themes/` directories or `metis.themes` entries in `package.json`
...
On first run, metis detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.metis/agent/themes
vim ~/.metis/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/earendil-works/metis/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, metis reloads it automatically for immediate visual feedback.
...
Metis uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, metis falls back to the nearest approximation.
