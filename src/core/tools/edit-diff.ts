...
function getEmptyOldTextError(path: string, editIndex: number, totalEdits: number): Error {
	const hint = "To append or insert, include a few existing lines of context in oldText, and those same lines along with your new content in newText.";
	if (totalEdits === 1) {
		return new Error(`oldText must not be empty in ${path}. ${hint}`);
	}
	return new Error(`edits[${editIndex}].oldText must not be empty in ${path}. ${hint}`);
}
