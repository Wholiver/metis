...
function rewriteNativeAgentPrompt(name: string, systemPrompt: string): string {
	let prompt = systemPrompt
		.replaceAll(
			"If you spawn, dispatch only a registered `*` persona and include this same activation and no-recursion contract.",
			"If you spawn, dispatch only admitted named workers for this lane. Children emit ChildResult and must not call performance_gate. Do not build an L0–L4 fleet.",
		)
		...
		.replaceAll("Report a tight result to the dispatcher", "Emit one ChildResult to host")
		.replaceAll("Report up to your dispatcher", "Emit one ChildResult to host/root")
		.replaceAll("Your dispatcher", "Host/root")
		.replaceAll("your dispatcher", "host/root")
		.replaceAll("Questions go to the dispatcher, never the user.", "Questions go to host/root in ChildResult; do not spawn a nested fleet.")
		.replaceAll(", not a dispatcher.", ", not a nested-fleet coordinator.")
		.replaceAll("**Parallelism is the dispatcher's job, not yours.**", "**Parallelism is host/root's job, not yours.**");
	if (name === "implementer") {
		prompt = prompt
			.replace(
				"You are the one L3 executor that may fan out: when the item has genuinely disjoint parts, you may spawn registered `*` L4 leaf personas for per-part attestation - spawn-all-then-collect with one distinct brief per leaf, never another implementer, and only where the brief names the leaf's exact duty. If the item contains independent implementation parts that exceed one executor's owned boundary, stop before editing and return a structured SPLIT-REQUEST naming each disjoint boundary and dependency to the coordinator or manager; only established L3 implementers may receive those implementation tracks. Otherwise sequence real dependencies yourself.",
				"Stay inside this admitted lane. Do not spawn extra leaf workers or a nested fleet. Sequence real dependencies yourself. If the item contains independent implementation parts that exceed this lane's owned boundary, stop before editing and return a structured SPLIT-REQUEST naming each disjoint boundary and dependency.",
			)
			.replace(
				/## First-L3 DISPATCH-row duty[\s\S]*?do not duplicate it\./,
				"Do not append GATELOG.md rows. The host records gates from ChildResult.",
			)
			.replace(
				"COMPLETE, PLAN-CONFLICT, or SPLIT-REQUEST",
				"ChildResult status completed|failed|blocked|invalid (COMPLETE text is not success), PLAN-CONFLICT, or SPLIT-REQUEST",
			);
	}
	if (name === "planner") {
...
function applyReliableHeadlessAgentProfile(config: ResolvedAgentConfig): ResolvedAgentConfig {
	...
	let systemPrompt = rewriteNativeAgentPrompt(config.name, config.systemPrompt.replace(PERFORMANCE_GATE_TOOL_CONTRACT, "")).trimEnd();
	if (!systemPrompt.includes("Reliable-headless ChildResult contract")) {
 ...
 * Custom named agents without a tools allowlist still omit `performance_gate`.
 */
const NAMED_AGENT_FALLBACK_TOOLS = [
	"read",
	"grep",
	"ls",
	"bash",
...
export function sessionToolsForNamedAgent(
	...
	void _env;
	if (resolvedTools && resolvedTools.length === 0) return [];
	const source = resolvedTools ?? NAMED_AGENT_FALLBACK_TOOLS;
	return source.filter((tool) => !NAMED_AGENT_STRIPPED_TOOLS.has(tool));
}

function bindBuiltinPromptToNativeRuntime(agent: AgentDefinition): AgentDefinition {
	return {
		...agent,
		systemPrompt: rewriteNativeAgentPrompt(agent.name, agent.systemPrompt),
	};
}
...
export function formatAgentsForPrompt(agents: AgentDefinition[]): string {
	...
	for (const agent of agents) {
		...
		const promptTools = (agent.tools ?? []).filter((tool) => tool !== "performance_gate");
		if (promptTools.length > 0) {
			lines.push(`    <tools>${escapeXml(promptTools.join(", "))}</tools>`);
		}
		lines.push("    <completion>ChildResult</completion>");
		if (agent.model) {
