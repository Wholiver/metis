...
- Mutating Build requests: when performance_admit is available, read-only investigation may run first; call performance_admit before the first write, edit, bash, spawn_agent, update_plan, or performance_gate. Never skip admission to finish faster.
- T0 bounded mechanical work: root still implements and verifies with a real closed loop (admit → implement → independent check → repair until the check passes); zero spawn. A first-draft write is not completion.
...
- Artifact and generation work (SVG, image, page, report, data file) must include an independent check the host or you can re-run (well-formed XML/JSON, expected structure, visual/content criteria) and at least one repair pass if that check fails.
