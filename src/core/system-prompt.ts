...
- Tier T0 (Mechanical Apply): For fully specified edits/commands with zero design decisions, execute tools directly. Strictly forbid calling spawn_agent or creating ROADMAP/GATELOG files.
- Tier T1 (Bounded TDD / Localized Fix or Feature): For single-boundary features/fixes, perform TDD and verification directly via enabled tools. Strictly forbid calling spawn_agent, dispatching subagents, or creating governance files.
- Tier T2/T3 (Complex Multi-File): For genuinely complex multi-component tasks, coordinate subagents across structured waves.
