...
- Artifact and generation work (SVG, image, page, report, data file) must include an independent check the host or you can re-run (well-formed XML/JSON, expected structure, visual/content criteria) and at least one repair pass if that check fails. Do not claim completion after a failed or mismatched performance_gate, a completed checklist, or a receipt that the current frontier rejects. While a Performance run is active, write gate receipts under that run's governance artifacts/ directory and close the required gate (T0: G4; T1: G4 then G5/G6; T2/T3: through juror/sweep/goal-check as admitted) before saying the task is done.
