...
- T0 bounded mechanical work: root still implements and verifies with a real closed loop (admit → implement → independent check → repair until the check passes); zero spawn. A first-draft write is not completion. Apply/T0 skips G0 and must close G4; a failed or mismatched performance_gate is not task success.
...
export function buildInstructionStack(options: BuildSystemPromptOptions): InstructionStack {
	...
	const turnBoundaryGuidance = "Write every visible intermediate text in the same language as the user's latest message. First think briefly and emit one concise visible intermediate text update before visible tool work begins. After that, emit another visible intermediate update only at a major milestone (phase change, failed check, or a decision that changes the next action). Never narrate one update per tool, and never filler such as '正在...', 'I'll...', or 'Wait, why...'. Do not put that progress only in thinking. When producing the final response or plan without tool calls, deliver the final answer directly without conversational meta-commentary, checklist recitation, or tag previews.";
