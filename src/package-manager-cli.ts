import { Markdown, type MarkdownTheme } from "@earendil-works/metis-tui";
...
async function getSelfUpdatePlan(force: boolean): Promise<SelfUpdatePlan> {
	let latestRelease: Awaited<ReturnType<typeof getLatestMetisRelease>>;
	try {
		latestRelease = await getLatestMetisRelease(VERSION);
	} catch (error: unknown) {
