...
import { InteractiveMode, runPrintMode, runRpcMode, runServerMode } from "./modes/index.ts";
import { initTheme, stopThemeWatcher } from "./modes/interactive/theme/theme.ts";
import { handleConfigCommand, handlePackageCommand } from "./package-manager-cli.ts";
import { isLocalPath, normalizePath, resolvePath } from "./utils/paths.ts";
import { cleanupWindowsSelfUpdateQuarantine } from "./utils/windows-self-update.ts";

const EXTENSION_LOAD_FAILURE_HINT = 'Hint: Start without extensions using "metis -ne".';
...
export async function main(args: string[], options?: MainOptions) {
	resetTimings();
	const offlineMode = args.includes("--offline") || isTruthyEnvFlag(process.env.METIS_OFFLINE);
	if (offlineMode) {
	...
	if (!existsSync(tempDir)) {
		try {
			...
			console.warn(`Failed to create .temp directory in ${cwd}:`, err);
		}
	} else {
		try {
			const now = Date.now();
			...
			for (const file of files) {
				...
				if (now - stats.mtimeMs > tenDaysMs) {
					rmSync(filePath, { recursive: true, force: true });
				}
			}
		} catch (err) {
			console.warn(`Failed to cleanup old files in .temp directory in ${cwd}:`, err);
		}
	}
	...
	if (await handlePackageCommand(args, { extensionFactories: options?.extensionFactories })) {
		...
		if (process.platform === "win32" && exitCode === 0 && args[0] === "update") {
			...
			// runs during teardown; let successful `metis update` drain naturally instead.
	...
	// Read piped stdin content (if any) - skip for RPC/server modes
	...
	const startupBenchmark = isTruthyEnvFlag(process.env.METIS_STARTUP_BENCHMARK);
	if (startupBenchmark && appMode !== "interactive") {
		console.error(chalk.red("Error: METIS_STARTUP_BENCHMARK only supports interactive mode"));
		process.exit(1);
	}
